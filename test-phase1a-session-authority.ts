...
import { roomRegistry } from "./server/engine-v2/registry/RoomRegistry";
import { dispatch } from "./server/engine-v2/reducer/dispatch";
import { sessionRegistry } from "./server/engine-v2/api/sessionRegistry";
...
// Join user 2
effects = dispatch(roomId, "user-2", {
  type: "JOIN_SESSION",
  payload: {
    userId: "user-2",
    displayName: "Bob",
...
const userSession = sessionRegistry.getUserSession("user-1");
...
if (room) {
  ...
  const remaining =
    room.timer.active && room.timer.endTime
...
// Check session phase
const roomAfterEnd = roomRegistry.getRoom(roomId);
if (roomAfterEnd) {
...
// Check session phase
const room2 = roomRegistry.getRoom(roomId2);
if (room2) {
...
const allSessionsAfter = sessionRegistry.getAllSessions();
console.log(`📊 Total sessions after termination: ${allSessionsAfter.length}`);

allSessionsAfter.forEach((session: any, i: number) => {
  console.log(`\n  Session ${i + 1}:`);
  console.log(`    ID: ${session.sessionId}`);
  console.log(`    Room: ${session.roomId}`);
  console.log(`    Phase: ${session.phase}`);
  console.log(
    `    Users: ${session.users.length} (${session.userNames.join(", ")})`,
  );
});
