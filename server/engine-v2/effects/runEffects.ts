...
function executeEffect(effect: Effect, io: Server): void {
  switch (effect.type) {
    ...
    case "GLIFF_APPEND": {
      ...
      console.log(
        `[runEffects] GLIFF_APPEND → room ${effect.roomId} | type: ${effect.entry.message.messageType}`,
      );
    ...
    case "EMIT_ROUND_STATE": {
      ...
      if (!tableState) {
        console.warn(
          `[runEffects] EMIT_ROUND_STATE: Room ${effect.roomId} not found`,
        );
        break;
      }
      ...
      console.log(
        `[runEffects] EMIT_ROUND_STATE → room ${effect.roomId} | Round ${roundData?.roundNumber || "none"}`,
      );
      break;
    }
    ...
    case "EMIT_READINESS_UPDATE": {
      ...
      if (!tableState) {
        console.warn(
          `[runEffects] EMIT_READINESS_UPDATE: Room ${effect.roomId} not found`,
        );
        break;
      }
      ...
      const readinessSummary = getReadinessSummary(
        ...
        tableState.participants,
      );

      io.to(effect.roomId).emit("round:readiness", readinessSummary);
      console.log(
        `[runEffects] EMIT_READINESS_UPDATE → room ${effect.roomId} | ${readinessSummary.ready}/${readinessSummary.total} ready`,
      );
      break;
    }
    ...
    case "EMIT_GLIFF_SNAPSHOT_TO_USER": {
      ...
      if (!effect.userId) {
        console.warn(
          "[runEffects] EMIT_GLIFF_SNAPSHOT_TO_USER: missing userId",
        );
        break;
      }
      ...
      if (snapshot.length > 0) {
        ...
        console.log(
          `[runEffects] EMIT_GLIFF_SNAPSHOT_TO_USER → socket ${effect.userId} | ${snapshot.length} entries`,
        );
      }
      break;
    }
