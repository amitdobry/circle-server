...
function setupSocketHandlers(io) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("request-join", ({ userId, name, avatarId, tableId }) => {
            ...
            try {
                ...
                if (roomState && roomState.participants) {
                    ...
                    const existingParticipant = roomState.participants.get(effectiveUserId);
                    if (existingParticipant &&
                        existingParticipant.presence === "CONNECTED") {
                        console.warn(`⚠️ [UserId Collision] userId "${effectiveUserId}" already in use by user "${existingParticipant.displayName}" → using socket.id instead`);
                ...
                console.warn("[UserId Validation] Could not check for duplicate userId:", error);
            }
            console.log(formatSessionLog(`📨 Join request: ${name} as ${avatarId} → table: ${resolvedTableId} (socket: ${socket.id}, userId: ${effectiveUserId})`, "INFO"));
        ...
        socket.on("content:vote-subject", (payload) => {
            ...
            (0, dispatch_1.dispatchAndRun)(roomId, userId, {
