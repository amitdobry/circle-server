 ...
 */
const delayedActionTimers = new Map();
/**
 ...
 */
function startSessionTimer(roomId, durationMs, io) {
    ...
    const timer = setTimeout(() => {
        ...
        const effects = (0, dispatch_1.dispatch)(roomId, null, {
            type: "TIMER_EXPIRED",
            payload: {},
        });
 ...
 */
function cancelSessionTimer(roomId) {
 ...
 */
function scheduleCleanup(roomId, delayMs, io) {
    // Cancel existing cleanup timer
    cancelCleanup(roomId);
    console.log(`[Cleanup] Scheduling cleanup in ${delayMs}ms for room ${roomId}`);
    const timer = setTimeout(() => {
 ...
 */
function runEffects(effects, io) {
    for (const effect of effects) {
        try {
            executeEffect(effect, io);
        }
        catch (error) {
            console.error("[runEffects] Effect execution failed:", effect.type, error);
        }
    }
}
/**
 ...
 */
function executeEffect(effect, io) {
    switch (effect.type) {
        // ========================================================================
        // SOCKET EMISSIONS
        // ========================================================================
        ...
        case "TIMER_START":
            ...
            console.log(`[runEffects] TIMER_START: ${effect.durationMs}ms for room ${effect.roomId}`);
            break;
        case "TIMER_CANCEL":
        ...
        case "DELAYED_ACTION":
            ...
            console.log(`[runEffects] DELAYED_ACTION: ${effect.action.type} in ${effect.delayMs}ms for room ${effect.roomId}`);
            break;
        // ========================================================================
        // ROOM LIFECYCLE
        // ========================================================================
        case "SCHEDULE_CLEANUP":
            ...
            console.log(`[runEffects] SCHEDULE_CLEANUP: ${effect.delayMs}ms for room ${effect.roomId}`);
            break;
        case "EXECUTE_CLEANUP":
            // Immediately cleanup room
            console.log(`[runEffects] EXECUTE_CLEANUP: Removing room ${effect.roomId}`);
        ...
        case "REBUILD_ALL_PANELS": {
            ...
            if (tableState) {
                const pointerEntries = Array.from(tableState.pointerMap.entries())
                    .map(([k, v]) => `${k}→${v}`)
                    .join(", ") || "(empty)";
                const connected = Array.from(tableState.participants.values())
                ...
                for (const [fromUserId, toUserId] of tableState.pointerMap) {
                    setPointer(fromUserId, toUserId, effect.roomId);
                    pointerSyncCount++;
                }
                console.log(`[REBUILD_ALL_PANELS] Synced ${pointerSyncCount} pointers from V2 → V1 (cache) in room ${effect.roomId}`);
        ...
        case "EMIT_READINESS_UPDATE": {
            ...
            if (!tableState.currentRound) {
                ...
                const activeCount = Array.from(tableState.participants.values()).filter((p) => p.presence === "CONNECTED").length;
                io.to(effect.roomId).emit("round:readiness", {
                    ready: 0,
                    total: activeCount,
                    readyUserIds: [],
                });
                console.log(`[runEffects] EMIT_READINESS_UPDATE (reset) → room ${effect.roomId} | round ended`);
                break;
            }
        ...
        // ========================================================================
        default:
            console.error("[runEffects] Unknown effect type:", effect.type);
            break;
    }
}
// ============================================================================
