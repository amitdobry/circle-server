...
exports.shadowDispatch = shadowDispatch;
exports.enableShadowMode = enableShadowMode;
exports.disableShadowMode = disableShadowMode;
 ...
 */
function shadowDispatch(roomId, userId, action) {
    try {
        ...
        console.error(`[V2 Shadow] ❌ ERROR in ${roomId} | ${action.type}:`, error.message);
...
function captureStateSnapshot(room) {
    const participants = Array.from(room.participants.values());
    return {
        phase: room.phase,
        liveSpeaker: room.liveSpeaker,
        connectedCount: participants.filter((p) => p.presence === "CONNECTED")
            .length,
        ghostCount: participants.filter((p) => p.presence === "GHOST").length,
...
// LOGGING
// ============================================================================
function logShadowDispatch(roomId, userId, action, before, after, effects) {
    ...
    console.log(`[V2 Shadow] 🎯 Room: ${roomId} | Session: ${sessionId.slice(0, 8)}... | User: ${userLabel} | Action: ${action.type}`);
    // Phase transition
    ...
    if (before.liveSpeaker !== after.liveSpeaker) {
        console.log(`  Speaker: ${before.liveSpeaker || "none"} → ${after.liveSpeaker || "none"} ✨`);
    }
    else if (after.liveSpeaker) {
        console.log(`  Speaker: ${after.liveSpeaker}`);
    }
    // Presence counts (highlight changes)
    if (before.connectedCount !== after.connectedCount ||
        before.ghostCount !== after.ghostCount) {
        console.log(`  Connected: ${before.connectedCount} → ${after.connectedCount} ✨`);
        console.log(`  Ghosts: ${before.ghostCount} → ${after.ghostCount} ✨`);
    }
    else {
        console.log(`  Connected: ${after.connectedCount} | Ghosts: ${after.ghostCount}`);
    }
    // Total participants
    if (before.participantCount !== after.participantCount) {
        console.log(`  Total Participants: ${before.participantCount} → ${after.participantCount} ✨`);
    }
    // Pointer map
    if (before.pointerMapSize !== after.pointerMapSize) {
        console.log(`  Pointers: ${before.pointerMapSize} → ${after.pointerMapSize} ✨`);
    }
    // Sync pause flag
...
function enableShadowMode() {
    ...
    console.log("[V2 Shadow] 🔍 Shadow mode ENABLED - V2 running as passive observer");
}
