...
exports.handlePurgeGhost = handlePurgeGhost;
const participantLifecycle_1 = require("../state/participantLifecycle");
function handlePurgeGhost(tableState, ghostUserId) {
    const effects = [];
    ...
    if (participant.presence !== "GHOST") {
        console.log(`[handlePurgeGhost] ✅ ${participant.displayName} reconnected, skipping cleanup`);
        return [];
    }
    // ========================================================================
    ...
    if (tableState.liveSpeaker === ghostUserId) {
        console.warn(`[handlePurgeGhost] ⚠️ Ghost ${participant.displayName} was still marked as speaker!`);
        for (const [fromUserId, toUserId] of tableState.pointerMap.entries()) {
    ...
    console.log(`[handlePurgeGhost] 🧹 Removing ${participant.displayName} (ghosted for ${ghostMinutes}m)`);
    // ========================================================================
    // STEP 3: Remove participant (handles speaker invalidation internally)
    // ========================================================================
    const result = (0, participantLifecycle_1.removeParticipantSafely)(tableState, ghostUserId, "PURGE_GHOST");
    // ========================================================================
    ...
    console.log(`[handlePurgeGhost] ✅ Purged ${participant.displayName} | Remaining: ${tableState.participants.size}`);
    // ========================================================================
