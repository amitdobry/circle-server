...
exports.INVARIANT_DESCRIPTIONS = void 0;
exports.assertInvariants = assertInvariants;
exports.assertInvariantsIfDev = assertInvariantsIfDev;
 ...
 */
function assertInvariants(tableState) {
    const { phase, liveSpeaker, syncPause, participants, pointerMap } = tableState;
    ...
    if (liveSpeaker !== null && !participants.has(liveSpeaker)) {
        throw new types_1.InvariantViolation(`liveSpeaker '${liveSpeaker}' does not exist in participants`);
    }
    ...
    for (const [pointerId, targetId] of pointerMap.entries()) {
        if (!participants.has(pointerId)) {
            throw new types_1.InvariantViolation(`pointerMap has pointer from '${pointerId}' but user not in participants`);
        }
        if (!participants.has(targetId)) {
            throw new types_1.InvariantViolation(`pointerMap points to '${targetId}' but user not in participants`);
        }
    }
    ...
    if (phase === "LIVE_SPEAKER" && liveSpeaker === null) {
        throw new types_1.InvariantViolation(`phase is LIVE_SPEAKER but liveSpeaker is null`);
    }
    ...
    if (phase === "SYNC_PAUSE" && syncPause !== true) {
        throw new types_1.InvariantViolation(`phase is SYNC_PAUSE but syncPause flag is false`);
    }
    // Invariant 8: Avatar Uniqueness per Room
    // No two CONNECTED or GHOST users can have the same avatarId
    const activeAvatars = new Map(); // avatarId -> userId
    for (const [userId, participant] of participants.entries()) {
        if (participant.presence === "CONNECTED" ||
            ...
            if (existingUser) {
                throw new types_1.InvariantViolation(`Avatar '${participant.avatarId}' is used by both '${existingUser}' and '${userId}'`);
            }
    ...
    if (liveSpeaker !== null) {
        ...
        if (speaker.presence !== "CONNECTED" && speaker.presence !== "GHOST") {
            throw new types_1.InvariantViolation(`liveSpeaker '${liveSpeaker}' has presence '${speaker.presence}', expected CONNECTED or GHOST`);
        }
    }
    ...
    const ghosts = Array.from(participants.values()).filter((p) => p.presence === "GHOST");
    if (connected.length === 0 && ghosts.length > 0) {
        if (phase !== "ENDING" && phase !== "ENDED") {
            throw new types_1.InvariantViolation(`All users are GHOST but phase is '${phase}', expected ENDING or ENDED`);
        }
    }
    ...
    if (phase === "ATTENTION_SELECTION" && liveSpeaker !== null) {
        throw new types_1.InvariantViolation(`phase is ATTENTION_SELECTION but liveSpeaker is set to '${liveSpeaker}'`);
    }
    // Invariant 12: LOBBY Initialization
    // If phase is LOBBY, liveSpeaker must be null and syncPause false
    if (phase === "LOBBY") {
        if (liveSpeaker !== null) {
            throw new types_1.InvariantViolation(`phase is LOBBY but liveSpeaker is set to '${liveSpeaker}'`);
        }
        if (syncPause !== false) {
            throw new types_1.InvariantViolation(`phase is LOBBY but syncPause is true`);
        }
    }
    ...
    if (phase === "TRANSITION" && liveSpeaker === null) {
        throw new types_1.InvariantViolation(`phase is TRANSITION but liveSpeaker is null (handoff requires outgoing speaker)`);
    }
    ...
    if ((phase === "ENDING" || phase === "ENDED") && liveSpeaker !== null) {
        throw new types_1.InvariantViolation(`phase is ${phase} but liveSpeaker is still set to '${liveSpeaker}'`);
    }
}
// ============================================================================
 ...
 */
function assertInvariantsIfDev(tableState) {
