...
exports.buildAttentionPanel = buildAttentionPanel;
const attentionCatalog_1 = require("../ui-config/attentionCatalog");
function buildAttentionPanel(ctx) {
    const others = ctx.participantNames.filter((name) => name !== ctx.userName);
    const participantButtons = others.map((name, index) => {
        const attention = attentionCatalog_1.attentionCatalog["attend-001"];
        return {
            id: `participant-btn-${index}`,
            type: "button",
            // panelType: "attentionPanel",
            buttonClass: "px-6 py-2 rounded-full text-sm font-semibold transition-all border bg-gray-100 text-gray-700 border-gray-300 hover:bg-gray-200",
            button: {
                label: name,
    ...
    const glowAttention = attentionCatalog_1.attentionCatalog["raise-hand"];
    const glowButton = {
        id: "ready-to-glow",
        type: "button",
        buttonClass: "px-6 py-3 rounded-full text-base font-semibold border transition-all duration-200 bg-indigo-400 text-white border-indigo-500 hover:bg-indigo-500 hover:shadow-md hover:scale-105 w-full sm:w-auto",
        // panelType: "attentionPanel",
        button: {
